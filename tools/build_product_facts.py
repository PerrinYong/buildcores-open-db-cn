...
def _extract_record(source_layer: str, category: str, file_path: Path, data: Dict[str, Any]) -> Dict[str, Any]:
...
def build_index() -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    if UPSTREAM_OPEN_DB_DIR.exists():
        source_dirs.append(("upstream", UPSTREAM_OPEN_DB_DIR))
    if CN_OPEN_DB_DIR.exists():
    ...
    processed = 0
    for source_layer, source_dir in source_dirs:
        print(f"Processing source: {source_layer} ({source_dir})")
        for category_dir in sorted([p for p in source_dir.iterdir() if p.is_dir()]):
            ...
            for json_file in category_dir.glob("*.json"):
                ...
                rec = _extract_record(source_layer, category, json_file, data)
                if not rec.get("opendb_id"):
                    invalid.append({"path": str(json_file), "error": "missing opendb_id"})
                    continue

                records.append(rec)
                by_source[source_layer] = by_source.get(source_layer, 0) + 1
